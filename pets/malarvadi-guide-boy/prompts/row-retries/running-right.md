Create Codex pet row `running-right` for `malarvadi-guide-boy`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Friendly child guide mascot based on the supplied Malarvadi boy: warm expressive face, dark tousled hair, bright blue shirt, green backpack, orange shorts, colorful sneakers, rounded 3D toy proportions, readable full body, no text or scenery.. Preserve silhouette, face, palette, material, proportions, markings, and props.
