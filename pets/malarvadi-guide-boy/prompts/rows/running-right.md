Create one horizontal animation strip for Codex pet `malarvadi-guide-boy`, state `running-right`.
...
Identity: same pet in every frame: Friendly child guide mascot based on the supplied Malarvadi boy: warm expressive face, dark tousled hair, bright blue shirt, green backpack, orange shorts, colorful sneakers, rounded 3D toy proportions, readable full body, no text or scenery.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `3d-toy`: Stylized 3D toy mascot with smooth rounded forms, simple materials, clear silhouette, and no photoreal complexity. User style notes: Keep a soft polished toy render with rounded plastic and fabric materials, bright primary colors, compact silhouette, simple clean chroma background..
