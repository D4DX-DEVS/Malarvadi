...
const MENU = [
  ...
  { href: "/about", label: "About Us", icon: UsersRound, tone: "nav-orange" },
  { href: "/programs", label: "Programs", icon: CalendarHeart, tone: "nav-amber" },
  { href: "/gallery", label: "Gallery", icon: Images, tone: "nav-lime" },
  { href: "/contact", label: "Contact Us", icon: Phone, tone: "nav-cyan" },
];
...
export function MobileFooterMenu() {
  ...
  return (
    <nav className="mobile-dock" aria-label="Footer menu">
      {MENU.map(({ href, label, icon: Icon, tone }) => (
        <a key={href} href={href} className={`${tone}${path === href ? " active" : ""}`} aria-current={path === href ? "page" : undefined}>
...
export function Header({ overlay = false }: { overlay?: boolean }) {
  const path = usePathname();
  // One logo colour per item; the tone class stays on even when active so the
  // dark active pill keeps the item's own accent ring.
  const link = (href: string, label: string, cls = "", alsoActiveOn: string[] = []) => {
    const active = path === href || alsoActiveOn.includes(path);
    return (
      <a
        key={href}
        href={href}
      ...
      <div className="wrap">
        <motion.div initial={{ y: -22, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: "spring", stiffness: 180, damping: 20 }} className="nav-shell">
          ...
          <nav className="nav-links">
            {link("/", "Home", "nav-red")}
            {/* About Us keeps its own page and opens a menu on hover or focus. */}
            <span key="about" className="nav-drop hide-m">
              {link("/about", "About Us", "nav-orange", ["/state-leaders"])}
              <span className="nav-drop-menu">
                {link("/state-leaders", "State Leaders", "nav-sub")}
              </span>
            </span>
            {link("/programs", "Programs", "nav-amber hide-m")}
            {link("/gallery", "Gallery", "nav-lime")}
...
export function PageHero({ kicker, title, sub, icon }: { kicker: string; title: string; sub: string; icon: React.ReactNode }) {
  return (
    <motion.div initial={{ opacity: 0, y: 22, scale: .985 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: .6 }} className="page-hero">
      ...
      <div><div className="kicker">{kicker}</div><h4>{title}</h4>{sub && <p>{sub}</p>}</div>
    </motion.div>
  );
}
